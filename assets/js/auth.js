...
document.getElementById('registerForm').addEventListener('submit', (e) => {
  ...
  localStorage.setItem('pendingUser', JSON.stringify({ companyName, email, password: pw }));
  document.getElementById('registerPage').style.display = 'none';
...
document.getElementById('setupGitHubBtn').addEventListener('click', async () => {
  ...
  if (!token || !repo) return alert('Fill in all fields');
...
function openTokenGuide() {
  alert('Go to GitHub → Settings → Developer Settings → Tokens → Generate new token with "repo" scope');
}
